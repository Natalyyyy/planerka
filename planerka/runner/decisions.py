...
def parse_decisions(text: str) -> dict[str, list[str]]:
    out, _ = _разобрать(text)
    return out


def строки_банка(text: str) -> list[dict[str, str]]:
    """Каждая классифицированная строка «Банка тем» — с сырым содержимым.

    `parse_decisions` отдаёт только очищенный текст темы (без `· откуда:
    источник` — тот отрезается для группировки по решению). Этого не
    хватает там, где важна вся строка целиком: например, идентификатор
    темы для callback_data должен различать две строки с одинаковым
    текстом темы, но разными источниками — а после очистки они неотличимы.

    Каждая запись: `{"состояние": ..., "тема": <очищенный текст, как в
    parse_decisions>, "содержимое": <всё после чекбокса, включая источник,
    без самого маркера>}`. Порядок и состав записей — те же самые «беру» /
    «подумать» / «нет» / «не_решено», что и в parse_decisions (общий проход
    по файлу, чтобы не разойтись с ним в разборе блоков кода и вложенных
    строк).
    """
    _, подробности = _разобрать(text)
    return подробности


def _разобрать(text: str) -> tuple[dict[str, list[str]], list[dict[str, str]]]:
