...
# --- Источник может быть одним файлом, а не только папкой ---
# Дефект: журнал сделанного живёт одним файлом «журнал-сделанного.md» в папке
# заметок, а collect требовал директорию и молча возвращал пустую строку.

def test_путь_на_файл_даёт_его_содержимое(tmp_path):
    (tmp_path / "журнал-сделанного.md").write_text("на неделе починил кран", encoding="utf-8")
    конфиг = {"источники": [{"имя": "журнал", "путь": "журнал-сделанного.md", "окно_дней": None}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert "на неделе починил кран" in out["журнал"]


def test_файл_источника_подписан_своим_именем(tmp_path):
    (tmp_path / "журнал-сделанного.md").write_text("строка", encoding="utf-8")
    конфиг = {"источники": [{"имя": "журнал", "путь": "журнал-сделанного.md", "окно_дней": None}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert out["журнал"].startswith("### журнал-сделанного.md")


def test_файл_источника_старше_окна_не_берётся(tmp_path):
    (tmp_path / "выгрузка – 2026-01-01.md").write_text("древность", encoding="utf-8")
    конфиг = {"источники": [{"имя": "статистика", "путь": "выгрузка – 2026-01-01.md", "окно_дней": 30}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert out["статистика"] == ""


def test_путь_в_никуда_пустая_строка_без_падения(tmp_path):
    конфиг = {"источники": [{"имя": "журнал", "путь": "нет-такого-файла.md", "окно_дней": None}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert out["журнал"] == ""


# --- Асимметрия «файл ↔ папка» по расширениям: она намеренная, фиксируем её ---

def test_прямой_путь_читает_не_только_md(tmp_path):
    (tmp_path / "выгрузка.csv").write_text("охваты;1000", encoding="utf-8")
    конфиг = {"источники": [{"имя": "статистика", "путь": "выгрузка.csv", "окно_дней": None}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert "охваты;1000" in out["статистика"]


def test_в_папке_берутся_только_md(tmp_path):
    (tmp_path / "встречи").mkdir()
    (tmp_path / "встречи" / "созвон.md").write_text("нужное", encoding="utf-8")
    (tmp_path / "встречи" / "черновик.txt").write_text("лишнее", encoding="utf-8")
    out = collect(_конфиг(окно=None), tmp_path, date(2026, 9, 5))
    assert "нужное" in out["транскрипты"] and "лишнее" not in out["транскрипты"]


def test_бинарник_по_прямому_пути_даёт_пустоту_а_не_падение(tmp_path):
    (tmp_path / "гайд.pdf").write_bytes(b"%PDF-1.4\n\x00\xff\xfe binary junk")
    конфиг = {"источники": [{"имя": "заметки", "путь": "гайд.pdf", "окно_дней": None}]}
    out = collect(конфиг, tmp_path, date(2026, 9, 5))
    assert out["заметки"] == ""
