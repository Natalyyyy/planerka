...
def test_скилл_даёт_исполнимую_команду_с_корнем_плагина():
    """Такты «темы» и «план» зовёт САМ Claude Code — внутри его вызова
    `${CLAUDE_PLUGIN_ROOT}` подставляется, и это правильная форма именно
    для них. Про команду «бот», которую человек запускает у себя в
    терминале, — тесты ниже: там переменная пуста."""
...
# --- команда «бот» в терминале человека
#
# `CLAUDE_PLUGIN_ROOT` подставляет Claude Code внутри своих вызовов. Бот —
# единственная команда, которую человек обязан запустить в СВОЁМ терминале:
# она не завершается, и держать её должен он сам. В терминале переменная
# пуста, и строка из документов превращается в
# `python3: can't open file '/run.py'` — живая проверка становится
# непроходимой на третьем шаге из восьми.

РЕПО = Path(__file__).resolve().parents[1]
ДОКУМЕНТЫ = {
    "README.md": РЕПО / "README.md",
    "docs/живая-проверка.md": РЕПО / "docs" / "живая-проверка.md",
    "skills/planerka/SKILL.md": ПЛАГИН / "skills" / "planerka" / "SKILL.md",
    "skills/planerka-setup/SKILL.md": ПЛАГИН / "skills" / "planerka-setup" / "SKILL.md",
}
ЗАПУСК_БОТА = re.compile(r'run\.py"?\s+бот\b')


def _документы_с_командой_бота() -> dict[str, str]:
    найдено = {}
    for имя, путь in ДОКУМЕНТЫ.items():
        текст = путь.read_text(encoding="utf-8")
        if ЗАПУСК_БОТА.search(текст):
            найдено[имя] = текст
    assert найдено, "ни один документ не показывает, как запустить бота"
    return найдено


def test_команда_бота_не_даётся_неподставленной_переменной():
    беда = []
    for имя, текст in _документы_с_командой_бота().items():
        for номер, строка in enumerate(текст.splitlines(), 1):
            if ЗАПУСК_БОТА.search(строка) and "CLAUDE_PLUGIN_ROOT" in строка:
                беда.append(f"{имя}:{номер}: {строка.strip()}")
    assert not беда, (
        "команду бота человек выполняет в своём терминале, где "
        "CLAUDE_PLUGIN_ROOT пуст:\n" + "\n".join(беда))


def test_каждый_документ_с_командой_бота_учит_узнать_настоящий_путь():
    """Настоящего пути не называл ни один файл репозитория — человеку было
    неоткуда взять то, что подставить вместо переменной."""
    молчат = [
        имя for имя, текст in _документы_с_командой_бота().items()
        if not ("echo" in текст and "CLAUDE_PLUGIN_ROOT" in текст)
    ]
    assert not молчат, (
        f"документ показывает запуск бота, но не говорит, где взять путь: {молчат}")
